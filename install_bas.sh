...
oc create -f bas-og.yaml 2>&1 >> "${logFile}"
...
oc create -f bas-subscription.yaml 2>&1 >> "${logFile}"
...
oc create secret generic database-credentials --from-literal=db_username=${dbuser} --from-literal=db_password=${dbpassword} -n "${projectName}" 2>&1 >> "${logFile}"

oc create secret generic grafana-credentials --from-literal=grafana_username=${grafanauser} --from-literal=grafana_password=${grafanapassword} -n "${projectName}" 2>&1 >> "${logFile}"
...
oc create -f analytics-proxy.yaml 2>&1 >> "${logFile}"
...
if [[ "${check_for_deployment_status}" == "Ready" ]]; then
	echoGreen "Full Deployment setup ready"
else
    echoRed "Full Deployment setup failed."
	exit 1;
fi
